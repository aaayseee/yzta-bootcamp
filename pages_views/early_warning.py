...
def render_early_warning_page(df_synthetic):
    ...
    st.dataframe(high_risk_df[['Tenure', 'Complain', 'DaySinceLastOrder', 'SatisfactionScore', 'RiskSkoru', 'ÖnerilenMüdahale']].head(15), width='stretch')
    ...
    with col_act1:
        ...
        if st.button("🚀 Kampanyayı Hemen Başlat", width='stretch'):
    ...
    with col_act2:
        ...
        st.plotly_chart(apply_plotly_theme(fig_radar), width='stretch')
        st.markdown('</div>', unsafe_allow_html=True)
